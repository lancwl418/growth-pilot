...
import { getChannelQuality } from "@/lib/queries/channel-quality";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await getChannelQuality(params.startDate, params.endDate);
    return jsonResponse(data);
  } catch (error) {
    if (error instanceof Error && error.name === "ZodError") {
      return errorResponse("Invalid date parameters", 400);
    }
    console.error("Channel quality API error:", error);
